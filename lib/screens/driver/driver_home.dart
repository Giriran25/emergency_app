...
class _DriverHomeState extends State<DriverHome>
  ...
  // Active Emergency Document Data
  String? _activeDocId;
  String _patientLocation = "Detecting Location...";
  String _emergencyType = "General Emergency";
  String _patientName = "Citizen";
  String _patientBlood = "--";
    ...
    FirebaseFirestore.instance
        ...
        .listen((snapshot) {
          if (snapshot.docs.isNotEmpty && mounted) {
            // Look for pending alerts or alerts accepted by current driver
            var currentUid = FirebaseAuth.instance.currentUser?.uid;

            var myAcceptedDocs = snapshot.docs.where((doc) {
              var data = doc.data();
              return data['driverId'] == currentUid &&
                  (data['status'] == 'accepted' ||
                      data['status'] == 'picked_up');
            }).toList();

            if (myAcceptedDocs.isNotEmpty) {
              var doc = myAcceptedDocs.first;
              var data = doc.data();
              setState(() {
                _activeDocId = doc.id;
                _hasActiveAlert = true;
                _isRequestAccepted = true;
                _isPatientPickedUp = (data['status'] == 'picked_up');
                _patientLocation = data['locationName'] ?? "MG Road Junction";
                _emergencyType = data['type'] ?? "Critical Alert";
                _patientName = data['citizenName'] ?? "Patient";
                _patientBlood = data['citizenBlood'] ?? "--";
              });
              return;
            }
            ...
            if (pendingDocs.isNotEmpty && !_isRequestAccepted) {
              var doc = pendingDocs.first;
              var data = doc.data();
              setState(() {
                _activeDocId = doc.id;
                _hasActiveAlert = true;
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  _infoRow(Icons.person, "Patient Name", _patientName),
                  const Divider(height: 30),
                  _infoRow(
                    Icons.medical_services_rounded,
                    "Emergency Type",
                    "$_emergencyType (Blood: $_patientBlood)",
                  ),
                  const SizedBox(height: 25),
                  if (!_isRequestAccepted)
                    _actionButton(
                    ...
                    _actionButton(
                      ...
                      () async {
                        if (!_isPatientPickedUp) {
                          ...
                          _snack("Patient Picked Up. Hospital Notified.");
                        } else {
                          if (_activeDocId != null) {
                            await FirebaseFirestore.instance
                                .collection('emergency_alerts')
